...
We record two facts that will be used numerous times for proving well-definedness of the inverse functor.
...
\begin{proof}
The challenge is showing that this is actually a model of the theory of flat functors on $FinProb$.  Now we check that the five axioms of the theory $\mathbb{T}^{FinProb}$ are satisfied, which is an arduous task that has already required quite a bit of setup in the form of \ref{interpretations}.
\begin{enumerate}
\item bla
\end{enumerate}
\end{proof}

...
\item We will use the shorthand $U_r := U_{r,r-1}$ for $r \in [0,1]$ and think of the functor $y U_r$ as the functor sending a finite probability space to the set of subsets of measure $r$.
\end{enumerate}
\end{definition}
...
Before continuing, we will derive a better criterion for determining whether a presheaf in $Set^{FinProb^{op}}$ is a sheaf. First, notice that we have the following universal property \footnote{In \cite{caramello_lafforgue} families of objects with these universal properties are called multilimits, see lemma 6.11.} :
...
\begin{theorem}
The atoms of the topos $\mathfrak{Prob}$ are given in the following list: ...
\end{theorem}
\begin{proof}
...
Geometrically, this means that for every $3$-dimensional matrix, if the sums projecting to two of the sides are in $R$, then so is the third.
\end{enumerate}
\end{proof}
To illustrate that these do not all correspond to atoms in our site, we calculate the easiest possible such failure.
\begin{example} Given $U_{\frac{1}{2},\frac{1}{2}}$, we generate the equivalence relation given the matrix $M:=\begin{bmatrix}
0& \frac{1}{2}\\
\frac{1}{2} &0
\end{bmatrix} $. It turns out to just be $R=\{
\begin{bmatrix}
 \frac{1}{2}&0\\
0& \frac{1}{2}
\end{bmatrix} ,
...
\end{example}
\begin{proof}
By reflexivity, all equivalence relations containing $M$, contain $R$. The relation is obviously symmetric. So it remains to show it is transitive. In the case of $2\times 2$ matrices, we can just solve linear equations, given $(t^{(1)}_{ij})$, $(t^{(2)}_{jk})$ to calculate $t_{ijk}$. Using this, it is easy to see that if $(t^{(1)}_{ij})=\begin{bmatrix}
0& \frac{1}{2}\\
\frac{1}{2} &0
\end{bmatrix}$ and $(t^{(2)}_{jk})= \begin{bmatrix}
 \frac{1}{2}&0\\
0& \frac{1}{2}
\end{bmatrix}$ then $(t_{ik}^{(3)})=\begin{bmatrix}
0& \frac{1}{2}\\
\frac{1}{2} &0
\end{bmatrix}$. If $(t^{(1)}_{ij})=(t^{(2)}_{jk})= \begin{bmatrix}
 \frac{1}{2}&0\\
0& \frac{1}{2}
\end{bmatrix}$ then $(t_{ik}^{(3)})= \begin{bmatrix}
\frac{1}{2}&0\\
0& \frac{1}{2}
\end{bmatrix}$. And finally, if $(t^{(1)}_{ij})=(t^{(2)}_{jk})=\begin{bmatrix}
0& \frac{1}{2}\\
\frac{1}{2} &0
\end{bmatrix}$  then $(t_{ik}^{(3)})= \begin{bmatrix}
\frac{1}{2}&0\\
...
We can use Theorem 7.3 (and remark 7.2) to see $(M,\alpha)$ is a special model of $\mathbb{T}_{int}$ and thus the universal model in $\text{Cont}_\tau(\text{Aut}(M,\alpha))$ is $(M,\alpha)$ with the canonical continuous group action.\newline 
...
The models we want to compute are given by the $J_{at}$-flat functors \linebreak $Flat_{J_{at}}(FinProb, Set)$ [reference?], so first we will use 
\[

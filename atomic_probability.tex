...
\]
HUH? WHAT THE FUCK IS HAPPENING HERE THIS LOOKS UNNECESSARY
...
\begin{corollary} \label{coprod_representation}
    $\mathfrak{Prob}\simeq \text{Sh}(\mathfrak{Prob}_{at},J_{at})$. More concretely, $\mathfrak{Prob}$ is the $\infty$-positivisation of $\mathfrak{Prob}_{at}$. % What is this even supposed to mean???
\end{corollary}
...
\section{Simple functions} \label{simple_functions}
...
We use \ref{atom_coprod_maps} to specify the operations $\mathcal{S} \times \mathcal{S} \to \mathcal{S}$ in a combinatorial fashion.

\begin{definition}
    \begin{enumerate}
        \item We will define addition on an element $(t_{ij}, y_i, z_j)$. Let $x_1 < x_2 < \cdots < x_k$ denote the elements of $\{y_i + z_j | i,j \}$. Then we define addition on elements to be
        \[
            (t_{ij},y_i,z_j) \mapsto (\sum_{y_i + z_j = x_k} t_{ij}, x_k) 
        \]
        and the corresponding map on atoms
        \[
            U_{t_{ij}} \to U_{\sum_{y_i + z_j = x_k} t_{ij}}
        \]
        to be the join the two pairs $(i,j)$, $(i',j')$ if the equality $y_i + z_j = y_{i'} + z_{j'}$ holds.
        \item We will define multiplication on an element $(t_{ij}, y_i, z_j)$. Let $x_1 < x_2 < \cdots < x_k$ denote the elements of $\{y_i \cdot z_j | i,j \}$. Then we define multiplication on elements to be
        \[
        ...
        \]
        to be the join the two pairs $(i,j)$, $(i',j')$ if the equality $y_i \cdot z_j = y_{i'} \cdot z_{j'}$ holds.
...
While insisting on defining the operations $+$ and $\cdot$ on atoms gives a purely combinatorial, but a bit long-winded, definition, the reader can check that these operations yield the usual pointwise addition and pointwise multiplication on the more intuitive sheaf $(X,\mu) \mapsto \mathbb{R}^X$ on $FinProb$. But the idea behind our longer definitions should be clear: When adding(or multiplying) two functions, we need to ensure that the values at each pair $(i,j)$ are distinct again and thus we need to join some of those pairs.
...
\end{proposition}
\begin{proof}
    The atomless measure algebra of $(X, \Sigma, \mu)$ is an inteval and thus can be viewed as a geometric morphism $F: Set \to \mathfrak{Prob}$. $F^*$ sends an atom $U_{r_i}$ to the set of partitions of the interval with measures $r_1,\cdots,r_n$. Thus, since $F^*$ is colimit preserving, $S$ gets sent to
    \[
        \coprod_{r_i \in \mathcal{A}^{re}} \{x_1 < \cdots x_n | x_i \in \mathbb{R}\} \times F^*(U_{r_1\cdots r_n}) .
    \]

...
\begin{corollary} $\mathfrak{Prob}\simeq \text{Sh}(FinProb_{at},J_{at})$. More concretely, $\mathfrak{Prob}$ is the $\infty$-positivisation of $FinProb_{at}$. % What is this even supposed to mean???
\end{corollary}
...
\begin{definition}
    \begin{enumerate}
        ...
        \]
        where we Yoneda embedd the objects of $FinProb_{at}$ into $\mathfrak{Prob}$ and then coproduct them.
    \end{enumerate}
\end{definition}
...
We give some examples as how to it makes things easier. Note that we omit wrting down the trivial subgroup in the triple below when the subsets of $\mathcal{A}$ we have are even subsets of $\mathcal{A}^{re}$.

\begin{example}
    \begin{enumerate}
        \item The binary multiproducts we have dealt with so often, can now be written as:
        \[ y U_{r_i} \times y U_{s_j} = \langle 1 | t_{ij} \in P^2_{r_i,s_j} \rangle, \]

...
\begin{theorem}
In the combinatorial interpretation of our topos we have that $A$ is interpreted as \\\[\bigsqcup_{\substack{r_1, \cdots, r_n> 0 \\ \sum_i r_i=1 \\V\subset Aut(U_{r_1\cdots r_n})\text{ subgroup}}} yU_{r_1\cdots r_n}/V\]
 the exponential is given by %expand the domain where this works by calculating the muliproducts of the atoms
 \[ (yU_{s_j})^{yU_{p_k}}(U_{r_i})=\prod_{\substack{\sum_i t_{ik}=p_k \\ \sum_k t_{ik}=r_i }} FinProb(U_{t_{ik}}, U_{s_j})=\bigsqcup_{\substack{\sum_i t_{ik}=p_k\\ \sum_k t_{ik}\neq 0}}yU_{\sum_k t_{ik}}\sqcup \cdots\]
\end{theorem}
\begin{proof}

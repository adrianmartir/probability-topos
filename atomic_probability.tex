...
We record two facts that will be used numerous times during \ref{classifying1}.
...
and
\[
\ldoub \vec{x}, y . f(\vec{x}) = y \rdoub_M \cong \{\vec{a}, \vec{b} . \theta_f^{\vec{a},\vec{b}} \} .
\]
\end{lemma}
...
We first will prove that $U$ is a model of $\mathbb{T}$. We begin with some well-definedness remarks. Notice that under the equivalence $Set^{FinProb^{op}} \simeq Set^{BPAlg_f}$, $U$ becomes the functor forgetting the boolean algebra structure. The operations $\wedge, \vee \text{ and } \neg$ on $U$ can now be defined as the regular boolean algebra operations at every object of $BPAlg_f$. Since the boolean algebra homomorphisms in $BPAlg_f$ preserve $\wedge, \vee \text{ and } \neg$, we get naturality of the operations. Similarly, the inclusions $y U_r \to U$ are natural by the fact that boolean probability algebra morphisms preserve measure.

Since we are looking at presheaves, all the colimit and limit conditions from the axioms can be checked pointwise, where they are trivial. More formally, we can use \cite{elephant}, Corollary D.1.2.14, to get the fact that $U$ is a $\mathbb{T}$-model in $Set^{BPAlg_f}$.

We abuse notation and identify the model $U$ with the corresponding functor $\mathcal{C}^{\mathbb{T}} \to Set^{FinProb^{op}}$. $U$, of course, induces a functor
\[
Set^{FinProb^{op}} \leftarrow Set[\mathbb{T}]: Lan_y(U).
\]

It remains to construct its inverse functor. We induce it by a geometric functor $\mathcal{C}^{\mathbb{T}^{FinProb}} \to \mathcal{C}^{\mathbb{T}}$, where $\mathbb{T}^{FinProb}$ denotes the theory of flat functors over $FinProb$. We give the functor in this way because it allows us to use internal logic to prove that the induced functor on classifying toposes is left exact, which will be needed when showing that the functors are inverses of each other. This geometric functor of course is given by a model of $\mathbb{T}^{FinProb}$ in $\mathcal{C}^{\mathbb{T}_{bpalg}}$. Let $f: (X,\mu) \to (Y, \nu)$ be an arrow in $FinProb$. We specify a model $M$ on the sort given by $(X,\mu)$ and on the function symbol given by $f$ as follows:
\begin{align*}
\{\vec{a} . \phi_{(X,\mu)}^{\vec{a}}\} &:= \Bigg \{ \vec{a} . \bigwedge_{x \in X} B_{\mu x} a^x \wedge \bigwedge_{\substack{x,y \in X \\ x \neq y}} (a^x \wedge a^y = 0) \Bigg \} \\
[\theta_f^{\vec{a}, \vec{b}}] &:= \Bigg [ \bigwedge_{y \in Y} (b^y = \bigvee_{f x = y} a^x) \wedge \phi_{(X,\mu)}^{\vec{a}} \wedge \phi_{(Y,\nu)}^{\vec{b}} \Bigg] ,
\end{align*}
so now we can give $M((X,\mu))$ by $\{\vec{a} . \phi_{(X,\mu)}^{\vec{a}}\}$ and $M(f)$ by $[\theta_f^{\vec{a}, \vec{b}}]$. The challenge will be showing that this is actually a model of the theory of flat functors on $FinProb$. The model $M$ has a straightforward intuitive meaning: The sorts can be interpreted as partitions of a boolean algebra into elements of prescribed measure. The function symbols compute unions of the partitions in prescribed ways. Now we get to the meat of the proof: we check that the five axioms of the theory $\mathbb{T}^{FinProb}$ are satisfied.
\begin{enumerate}
\item bla
\end{enumerate}

We still need to check that the functors we defined are inverses to each other.
...
\]
But this can be checked by applying the sheaf condition to the cover of $\{b . \top\}$ given by the $\{b . B_r b\}$.
...
\begin{tikzcd}[column sep=tiny]
    {y \{b_1,\cdots,b_n . \top \}} \arrow[d, "{[f(b_1,\cdots,b_n) = a]}"'] \arrow[r, "\cong"',phantom, shift right=7] & (Lan_F U)^n \arrow[d, "Lan_F U(f)"] \\
    y \{a . \top \}                                                                                           & Lan_F U                            
\end{tikzcd}
\]
in the category of arrows in $Set[\mathbb{T}]$. We also need the analogous statement for relation symbols.

We first consider the 0-ary function $0$. On $U$ it is the map $0: * \to \coprod y U_r$. Notice that $FinProb$ also has a terminal object $*$ and thus $y * \cong *$. In order to send the map $0$ through $Lan_F$, we write it as the composition $y * \to y U_0 \to U$. Applying $Lan_F$ now yields the composition
% \[ Why does xrightarrow draw such scuffed arrows????
% y \{[] . \top\} \xrightarrow{[\top]_*} y \{a . B_0 a \} \xrightarrow{[(a=b) \wedge B_0 a]_*} y \{ b . \top \}
% \]
\[
\begin{tikzcd}[column sep=huge]
    y \{[] . \top\} \arrow[r, "{[\top]_*}", "\cong"'] & y \{a . B_0 a \} \arrow[r, "{[(a=b) \wedge B_0 a ]_*}"] & y \{b . \top \}
\end{tikzcd}
\]
...
\begin{align*}
    [(a=b) \wedge B_0 a]_* [\top]_* &= [\exists a . (a = b) \wedge B_0 a]_* \\
    &= [B_0 b]_* \\
    &= [b = 0]_* ,
\end{align*}
which is precisely the $0$-map for the universal model. The proof for $1$ is almost the same, again one only needs the fact that $B_1 a \dashv \vdash_a (a = 1)$, which is provable in our theory.

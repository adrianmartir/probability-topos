...
\[ \mathcal{F} :(N,\nu)\mapsto\text{FinProb}((M,\mu),(N,\nu))/\sim  \]
where $f\sim g$ if there exists $\sigma\in U$ with $f\circ \sigma=g$. These are partitions up to permutation by elements in $U$.
\end{lemma}
\begin{proof}
One calculates coequalizers in sheaf topoi by calculating the section-wise coequalizer and then sheafifying. Sheafification is not needed in our case however as the presheaf given in the statement is a sheaf: IS THIS TRUE? We seek to apply the criterion \ref{sheaf_condition_weak}. We need to check that the following is an equalizer diagram for any $f:(Z,\lambda)\rightarrow (Y,\nu)$:
\[
...
\end{tikzcd} ,
\]
%This should work. It should still force it to be constant on fibers. And the same up to permutation making it lift.
...
\end{proof}
We are also equipped to characterise this category in combinatorial term.

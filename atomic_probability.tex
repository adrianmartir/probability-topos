...
\end{tikzcd} ,
\]
For any measure-preserving map $g: (Z,\lambda) \rightarrow (X,\mu)$ such that $g\circ \pi_1=\sigma \circ g\circ \pi_2$ for some $\sigma\in U$, we see that $f^{-1}\{y\} \cap g^{-1}\{x\} =f^{-1}\{y\} \cap g^{-1}\{\sigma^{-1}x\} \in \{f^{-1}\{y\} ,\emptyset \}$ for any $y\in Y$ and $x\in X$. Hence we can define $\tilde{g}: (Y,\nu)\rightarrow (X,\mu)$ via $y\mapsto x$ for some choice of $x$ with $f^{-1}\{y\}\subseteq g^{-1}\{x\} $. This yields a map $ \mathcal{F} (Y,\nu)\rightarrow Eq(\pi_1, \pi_2)$ as the maps on partitions are all $U$-equivariant. Evidently, the definition of $\tilde{g}$ is unique up to permutation by $\sigma\in U$. Hence the map is an isomorphism.
\end{proof}

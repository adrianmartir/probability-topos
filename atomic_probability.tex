...
\begin{proof}
    We need to show functoriality and then verify that $Lan_F$ is left-exact. The latter property is checked by the usual identification of functors whose left Kan extension is left-exact with functors that are flat. Concretely, we use theorem \Rmnum{7}.9.1 in \cite{sheaves_geometry_logic}(page 399). The cited theorem says that flat functors(defined as functors whose left Kan extension is left-exact) precisely correspond to the functors satisfying the straightforward list of properties listed in the definition of a filtering functor, which is defined in \Rmnum{7}.8.1 (page 394).

    The reader who actually reads the definition of a filtering functor will quickly notice that that definition can be checked in our case by showing that $F$ preserves the terminal object, binary multiproducts and multiequalizers in the sense described below.
    ...
    \begin{enumerate}
        ...
        \item We show that $F$ preserves composition. Given morphisms $f: \alpha \to \beta$, $g: \beta \to \gamma$ in $FinProb$,
            \begin{align*}
                F(g)F(f) &= [\exists c . \theta^{\vec{a},\vec{c}} \wedge \theta^{\vec{c},\vec{b}}_g] \\
...
\end{proof}
Combining this statement with the previous lemma, we get that the validity of atomic statement does not depend on the atomless probability algebra we choose. Simply because they only depend on the joint cumulative distribution function and the set of these functions coincides for all choices. Let's extend this result! Before that we need one more technical result to deal with quantifiers.
\begin{lemma} Given a function $g:\mathbb{R}^n\rightarrow \mathbb{R}$ with the conditions of lemma \ref{distribution_L_infty} and bounded measurable functions $f_1,\cdots, f_{n-1}\in L^\infty(X,\mu)$ for some atomless probability algebra, such that their joint cumulative distribution function $\tilde{g}:\mathbb{R}^{n-1}\rightarrow \mathbb{R}$ has
...
\end{lemma}
\begin{proof}
We will simply construct such a function. 
\end{proof}
\begin{theorem}
For any infinitary first order statement in the theory of expectation algebras $\Phi$ and all atomless probability algebras $(X,\mu)$, $(Y,\nu)$, we have:
...
\end{theorem}
\begin{proof}
Note that we can uniquely define the set of $n$-tuple of measurable functions with a given cumulative distribution function of some $n$-tuple $f_1,\cdots,f_n$ by $ $
First, we treat the case of quantifier free formulas. Via a simple symbolic recursion, the previous lemma \ref{atomic_L_infty} implies that the validity also only depends on the joint cumulative distribution function. It remains to check what happens for quantification. We proceed by symbolic recursion:\newline
\indent Assume one has a statement $\Phi(f_1,\cdots f_n)$ with free variables $f_1,\cdots, f_n$ which only depends on the joint cumulative distribution of $f_1,\cdots, f_n$. We seek to show that then, so does $\exists f_1.\Phi(f_1,\cdots f_n)$. LAST PUZZLE PIECE\newline

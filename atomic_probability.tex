...
\begin{theorem} The category of points of the topos $\textbf{Sh}(FinProb,J_{at})$ is equivalent to the full subcategory of $BProbAlg$ of boolean probability algebras $(B,\mu)$ such that for each $r\in (0,1)$ and each $b\in B$ there exist $b', b''\in B$ with $b'\vee b''=b$, $b'\wedge b''=0$ and $\mu(b')=r\mu(b)$.\end{theorem}
...
Note: There are no countable Set-based models. This is about the measure-theoretic continuum.

\begin{definition}
Define the geometric theory $\mathbb{T}$ of boolean probability algebras to have one sort $B$, constants and function symbols
\begin{center}
$1: B$ \\
$0: B$ \\
$\wedge: B \times B \to B$ \\
...
We have seen that $Set^{FinSet^{op}}$ is really about statements about a measure-theoretic notion of a continuum. Traditionally, the interval with the Lebesgue measure has played this role. In this section we will see that the interval equipped with the Lebesgue measure already understands all statements about the continuum probability algebras. In fact, this interval is not special. Our proof will work precisely the same for (all?) continuum probability algebras that are complete(are they always complete?). (Are all continuum probability algebras surjective points?). Also: When do we need choice?

I would hope for a positive answer to the questions above and interpret it as follows: Using our construction, we have derived an axiomatization of a measure-theoretic continuum that for the many intents and purposes behaves like the interval. The most interesting thing, is that the probability theoretic intuition of the choice of base measure space being irrelevant for all intents and purposes, now gets a mathematical formalism. Only that now, by requiring extension properties in the atomic topology, we saw that we might as well stop talking about non-continuum probability algebras. Talking about them would allow us to talk about specific atoms, which is undesirable(Some Tao extension stuff here).

So in a very real way, the study of $Set^{FinSet^{op}}$ is precisely the study of a very well-behaved continuum. Thus, it is reasonable to expect being able to do probability theory based on this topos. One should think about $Set^{FinSet^{op}}$ as some sort of universal probability space representing the continuum.

...
\begin{theorem}
In the combinatorial interpretation of our topos we have that $A$ is interpreted as \\\[\bigsqcup_{\substack{0<r_1\leq  \cdots\leq r_n \\ \sum_i r_i=1 \\V\subset Aut(U_{r_1\cdots r_n})\text{ subgroup}}} yU_{r_1\cdots r_n}/V\]%it would be better to choose one subgroup per conjugacy class
 the power object is given by %expand the domain where this works by calculating the muliproducts of the atoms
 \[ \Omega^{yU_{s_j}/G}(U_{r_i},H)=\mathcal{P}\{ (t_{ij})| \sum_i t_{ij}=s_j,\, \sum_j t_{ij}=r_i,\, t_{ij}\geq 0 \} \]%does the group play in here?
...
\end{theorem}
\begin{proof}
The statement on the exponential is Proposition 1 of III6 \cite{sheaves_geometry_logic}.
\end{proof}

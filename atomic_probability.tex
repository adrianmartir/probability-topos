...
One immediately sees that this is a polynomial in $\mathbb{R}[\lambda_{ij}, \mu(a_j)]_{1\leq i\leq n, 1\leq j \leq m}$. Together with the restriction, that the $a_j$ are a partition of unity i.e. $\mu(a_j)> 0$ and $\sum_j \mu(a_j)=1$, this forms a system of polynomial equations. Call the semi-algebraic set of solutions $\mathcal{S}_{\phi,m}\hookrightarrow (\mathcal{S}^n)_m$. Hence we find, that $\ldoub \phi \rdoub_\mathcal{S} \hookrightarrow \mathcal{S}^n$ can be seen as the countable disjoint union of semi-algebraic sets $\bigcup_{m\in \mathbb{N}} \mathcal{S}_{\phi,m}$. If one weakens the condition to $\mu(a_j)\geq 0$, it is evidently even a increasing union. % symbolically, these all only depend on f, so intuitively one should be able to apply Seidenberg-Tarski. But this isn't super straight forward, sadly
\end{proof}

\begin{theorem}[Seidenberg-Tarski] Every infinitary-first-order statement $\phi$ in the signature of $\mathbb{T}_{exp}$ is equivalent to a (possibly) infinite disjunction of atomic statements.
\end{theorem}
\begin{proof}
We use symbolic recursion. $\top$, $\neg$ and $\vee$ are clear. It remains to prove the statement for $(\exists x) (\phi(x,y_1,\cdots, y_n))$ where $\phi$ is an atomic statement. Again, we have
\begin{center}
\begin{tikzcd}
\ldoub\phi\rdoub_\mathcal{S} \arrow[r, hook] \arrow[d, two heads] & \mathcal{S}^{n+1} \arrow[d, "\pi" description] \\
\ldoub (\exists x)\phi \rdoub_\mathcal{S} \arrow[r, hook]         & \mathcal{S}^n                                 
\end{tikzcd}
\end{center}
On levels $(\mathcal{S}^n)_{m}$ this yields the equations $\exists \lambda_1\cdots \exists \lambda_m. \sum_{j=1}^m f(\lambda_j,\lambda_{2,j},\cdots \lambda_{n,j})\mu_j=0$. Call this polynomial $p_0$ and call $p_{0,1}:=f(\lambda_1,\cdots, \lambda_{n,1})$. Now we use quantifier elimination for real closed fields. For this, we need to differentiate the formula w.r.t. $\lambda_1$ yielding:
\[ \mu_1\cdot p_{1,1}:=\frac{\partial}{\partial \lambda_1} \sum_{j=1}^m f(\lambda_j,\lambda_{2,j},\cdots \lambda_{n,j})\mu_j= \mu_1 \frac{\partial}{\partial \lambda_1}f(\lambda_1,\lambda_{2,1}\cdots \lambda_{n,1}) \]
The remainder (denote it rem) when dividing is 
\[\mu_1 \text{rem}(p_{0,1},p_{1,1})+\sum_{j=2}^{m}f(\lambda_j,\lambda_{2,j},\cdots \lambda_{n,j})\mu_j\]
Call the first term of sum $\mu_1 \cdot p_{2,1}$. Note that $\text{rem}(p_{1,1},p_{2,1})=0$ unless\linebreak
 $\sum^m_{j=2}f(\lambda_j,\lambda_{2,j},\cdots,\lambda_{n,j})\mu_j=0$. This reduces to the case $\exists \lambda_2 \cdots \exists \lambda m.$ \linebreak $\sum^m_{j=2}f(\lambda_j,\lambda_{2,j},\cdots,\lambda_{n,j})\mu_j=0$. Keeping this in mind, we consider the case $m=1$. Here we use Seidenberg-Tarski to yield polynomials $g_1,\cdots, g_k$ and $h_1,\cdots h_l$ in $\lambda_2,\cdots, \lambda_n$ with $g_i(\lambda_2,\cdots, \lambda_n)> 0$ and $h_i(\lambda_2,\cdots, \lambda_n)=0$ iff $\exists \lambda_1.f(\lambda_1,\cdots,\lambda_n)=0$.
\end{proof}

...
\begin{abstract}
Yo guys we solved probability theory. %TODO do this but properly
\end{abstract}
\end{minipage}
\end{center}
\tableofcontents 
...
\end{center}
\end{definition}
\begin{proposition} The internal language of $\mathfrak{Prob}$ is a classical model of ZFA.
\end{proposition}
\begin{proof} Our presentation follows [ref: sheaf models for set theory] chapter 2 and 3, where much more detail can be found. It is also proven there, that the structure we will give in this proof actually fulfils the axioms. \newline%later we will also use chapter 5
\indent
We use the site presentation $\mathfrak{Prob}\simeq \text{Cont}_\tau(\text{Aut}_{BPAlg})(I,\lambda)$.\newline
\indent In any of the continuous group action, the subobject classifier resp. natural numbers object is given by the two element set resp. $\mathbb{N}$ with trivial actions. The exponentials of group actions $X$ by $Y$ are computed as sets of functions $X\rightarrow Y$ whose stabilizer under the action $g.f(x)=g.f(g^{-1}.x)$ is open. $A$ is interpreted as $\bigsqcup_{gH\text{is the set of cosets of open subgroup}} gH$. %This seems incorrect, we want the set of cosets
...
\indent Now one uses an analogue of the cumulative hierarchy, starting with the set of atoms and then taking power objects and directed colimits just as one would in set-theory.
\end{proof}
...
where the last step follows as atoms only have $2$ subsheaves, themselves and the trivial one. Note that the data of the subgroup can be ignored here as it is canonically induced. To decompose it into atoms, we need to find all its subsheaves, which are atoms %TODO outsource this proof into the previous section
\end{proof}

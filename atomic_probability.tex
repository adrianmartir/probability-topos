...
% TODO: The bridge (including the inverse functor!) should be made more explicit in the statement of the theorem. Probably one should move the flat functor proof to another lemma.
...
\end{align*}
so now we can give $M((X,\mu))$ by $\{\vec{a} . \phi_{(X,\mu)}^{\vec{a}}\}$ and $M(f)$ by $[\theta_f^{\vec{a}, \vec{b}}]$. The challenge will be showing that this is actually a model of the theory of flat functors on $FinProb$. The model $M$ has a straightforward intuitive meaning: The sorts can be interpreted as partitions of a boolean algebra into elements of prescribed measure. The function symbols compute unions of the partitions in prescribed ways. Now we check that the five axioms of the theory $\mathbb{T}^{FinProb}$ are satisfied, which is an arduous task.
\begin{enumerate}
\item bla
\end{enumerate}
...
(This must be cleaned up a bit) To show that $Lan_F U(\neg)$ matches the corresponding operation on the universal model, we show that morphisms of distributive lattices internal to a topos $\mathcal{E}$ automatically preserve $\neg$. So let $M$, $N$ be boolean algebras internal to $\mathcal{E}$ and let $\varphi: M \to N$ be a homomorphism of distributive lattices. We have that
\[
[y=\neg x] = [(x\wedge y=0) \wedge (x\vee y=1)] ,
\]
where we view both formulas as $\mathbb{T}$-provably functional formulas $\{x.\top\} \to \{y.\top\}$ in the syntactic category of boolean algebras. Thus, by identifying boolean algebras with functors from the syntactic category of boolean algebras into $\mathcal{E}$, the diagram
\[
\begin{tikzcd}
M \arrow[d, "M(\neg)"'] \arrow[r, "\varphi"] & N \arrow[d, "N(\neg)"] \\
M \arrow[r, "\varphi"]                       & N                     
\end{tikzcd}
\]
must already commute.

Luckily, sending $U_r \to \coprod U_r$ through $Lan_F$ is easily seen to give the expected subobject, concluding the proof.
\end{proof}

\begin{theorem}
This extends to our results about probability intervals.
\end{theorem}

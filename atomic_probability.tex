...
\]
are in one-to-one correspondence with elements of the multiproduct. These matrices form a polytope inside $\mathbb{R}^{n\times m}$ called transportation polytope. Any element of a polytope is a convex sum of its extreme points. These and much more about this polytope are explored in Chapter 8 of [Matrix book]. \newline
\indent $yU_{r_i}\times_{yU_{s_i}}yU_{r'_i}$ corresponds (after reordering the elements appropriately) to the subset of matrices in the block diagonal from with non-zero entries at $(i,j)$ only if $g(i)=f(j)$.
\end{remark}
...
\begin{definition} An \emph{atom} of a topos is an object whose only subobjects are trivial or itself.
\end{definition}
This concept is very relevant, as any sheaf in an atomic topos is a coproduct of atoms \cite{XYZ}. This means, that all information contained in the topos is already given by its atoms. If one understands them well, it is quite easy to calculate things in an atomic topos.
\begin{definition} The \emph{category of atoms} of a topos is the full subcategory of objects, which are atoms.
\end{definition}
\begin{theorem}
...
More concretely, $\mathcal{E}$ is the $\infty$-positivisation of $\mathcal{E}_{at}$ i.e. the free completion w.r.t. small coproducts.
\end{theorem}
\begin{proof}
...
We will utilize the combinatorial method outlined in \cite{caramello_lafforgue} Prop 6.7. For this, we will need to understand internal equivalence relations on the sheaves $yU_{r_1\cdots r_n}$ as the atoms will be reconstructed as quotients of these relations. We use \ref{multipullback} to yield the following description of an equivalence relation. \newline
...
%Again, fix a partition $A_1\cup\cdots\cup A_m=\{1,\cdots, n\}$ into non-empty disjoint sets. Additionally, fix a subgroup $H$ of $Aut(U_{s_i})$, for $s_1, \cdots, s_m$ with
%\[
%    s_i = \sum_{j \in A_m} r_j ,
%\]
%and where $Aut$ denotes the automorphisms in $FinProb$.
...
\end{lemma}
\begin{proof}
One calculates coequalizers in sheaf topoi by calculating the section-wise coequalizer and then sheafifying. The quotient the equivalence relation $R$ associated to $U$ is given by the coequalizer of the composition of the  maps%Maybe explain previously what they quotients of equivalence relations are
\begin{center}
...
Before we actually use $FinProb_{at}$, we describe this category in a straightforward combinatorial manner.

\begin{theorem} The category $FinProb_{at}$ is equivalent to the following category. \begin{enumerate}
...
\begin{corollary} \label{coprod_representation}
    $\mathfrak{Prob}\simeq \text{Sh}(FinProb_{at},J_{at})$. More concretely, $\mathfrak{Prob}$ is the $\infty$-positivisation of $FinProb_{at}$. % What is this even supposed to mean???
\end{corollary}
...
\begin{definition}
    \begin{enumerate}
        ...
        \[
            \{((r_i)_{i=1,\cdots, n}, H) | r_i \geq 0, \sum r_i = 1, H \leq Aut(U_{r_i})\}.
        \]
        ...
        \[
            \{(r_i)_{i=1,\cdots, n} | r_i \geq 0, \sum r_i = 1\}.
        \]
        ...
        \]
        where we Yoneda embedd the objects of $FinProb_{at}$ into $\mathfrak{Prob}$ and then coproduct them. We will call a sheaf written in this notation a \emph{set of atoms}.
    \end{enumerate}
\end{definition}
...
\begin{example}
    \begin{enumerate}
        ...
        \[ y U_{r_i} \times y U_{s_j} = \langle 1 | t_{ij} \in P^2_{r_i,s_j} \rangle, \] % TODO: Define the P stuff earlier.
        where $1$ denotes the one-object set
        \item We can also express the universal model $U$ in the following way:
        \[ U = \langle 1 | (r_1,r_2) \in P^1_1 \rangle. \]
    \end{enumerate}
\end{example}

The previous examples may be too easy to fully make use of the notation. In chapter \ref{applications} we will see that describing the simple functions will heavily rely on it. For that example, we will also need to understand maps between sets written in our notation.

One of the most intriguing things about this notation, is that it almost feels like it gives semantics to an abstract notion of \emph{sets parameterized by a partition of unity}, defined in the formal sense above. The striking thing, is that naively defining such a concept of a parameterized set, would most likely end in failture, for the simple reason that on an elementary level(if one were to expand all the topos theory), these semantics are ridiculously complicated. Especially because of the fact that there exist 'imaginary atoms' which took considerable efforts to classify.

\begin{proposition}
    Describe maps between two sets in this notation $\langle .. \rangle$.
\end{proposition}
...
In the combinatorial interpretation of our topos we have that $A$ is interpreted as \\\[\bigsqcup_{\substack{0<r_1\leq  \cdots\leq r_n \\ \sum_i r_i=1 \\ [V]\text{ conjugacy class of subgroup }V\subset Aut(U_{r_1\cdots r_n})}} yU_{r_1\cdots r_n}/V\]%it would be better to choose one subgroup per conjugacy class
 the power object is given by %expand the domain where this works by calculating the muliproducts of the atoms
 \[ \Omega^{yU_{s_j}/G}(U_{r_i},H)=\mathcal{P}\{ (t_{ij})| \sum_i t_{ij}=s_j,\, \sum_j t_{ij}=r_i,\, t_{ij}\geq 0 \} \]%does the group play in here? Most likely incorrect
 decomposing into atoms as
 \[ \Omega^{yU_{s_j}} =\bigsqcup_{\substack{\sum_i t_{ik}=p_k\\ \sum_k t_{ik}\neq 0}}yU_{\sum_k t_{ik}}\sqcup \cdots   \]
 with element-hood relation given by:
\[ \in=\bigsqcup_{subset}\hookrightarrow \bigsqcup=yU_{s_j}/G\times \Omega^{yU_{s_j}/G}\]
\end{theorem}
\begin{proof}
The statement on the exponential is Proposition 1 of III6 \cite{sheaves_geometry_logic}. We have
 \[ \Omega^{yU_{s_j}/G}(U_{r_i},H)=\text{Hom}(yU_{s_j}/G\times yU_{r_i}/H, \Omega) =\]
 \[\text{Hom}(\bigsqcup_{\substack{ \sum_i t_{ij}=s_j,\\ \sum_j t_{ij}=r_i,\\ t_{ij}\geq 0 \\ K \text{ maximal subgroup with property P}}}yU_{t_{ij}}/K,\Omega)=\prod_{"-"} \text{Hom}(yU_{t_{ij}}/K,\Omega)=\prod_{"-"}2  \]
where the last step follows as atoms only have $2$ subsheaves, themselves and the trivial one. Note that the data of the subgroup can be ignored here as it is canonically induced. To decompose it into atoms, we need to find all its subsheaves, which are atoms
\end{proof}
...
\[U^*\dashv U_*: \text{Sh}(FinProb, J_{at})\rightarrow  \text{Sh}(FinSur, J_{at})\]
 ...
 \[U^*yX=\bigsqcup_{\mu\text{ is a measure on }X} y(X,\mu)\]%mention explicit description of maps
\end{lemma}
...
\begin{center}
\begin{tikzcd}
{U^*yX(Z,\lambda)} \arrow[d, "{U^*y(f)(Z,\lambda)}"'] \arrow[r, dash] & {FinSur(Z,X)} \arrow[d, "f\circ -" description]\arrow[r, dash]  & {\bigsqcup_{\mu}FinProb((Z,\lambda),(X,\mu))} \arrow[d, "\alpha_f (Z{,} \lambda)"] \\
{U^*yY(Z,\lambda)} \arrow[r, dash]                                   & {FinSur(Z,Y)}\arrow[r, dash]                    & {\bigsqcup_\nu FinProb((Z,\lambda),(Y,\nu))}                     
\end{tikzcd}
\end{center}
...
\[\simeq \prod_{\mu \text{ is a measure on }X}\text{Hom}(y(X,\mu),\Omega)\simeq \prod_{\mu \text{ is a measure on }X} 2\simeq 2^{\{\mu \text{ is a measure on }X\} } 
 ...
 \begin{center}
 \begin{tikzcd}
{\text{Hom}(U^*yY,\Omega)} \arrow[d, "-\circ U^*y(f)"'] \arrow[r,dash] & {\text{Hom}(\bigsqcup_\nu y(Y,\nu),\Omega)} \arrow[d, "-\circ \alpha_f" description] \arrow[r,dash]  & {\text{Sub}(\bigsqcup_\nu y(Y,\nu))} \arrow[d, "\text{pull back along }\alpha_f"] \\
{\text{Hom}(U^*yX,\Omega)} \arrow[r,dash]                                        & {\text{Hom}(\bigsqcup_{\mu} y(X,\mu),\Omega)}          \arrow[r,dash]                                & {\text{Sub}(\bigsqcup_\mu y(X,\mu))}                                             
\end{tikzcd}
\end{center}
...
%[quote some paper] We can recover $\mathbb{M}$ as the following composite:
%\begin{center}
%\begin{tikzcd}
...
\indent Thus $U$ is a complete boolean algebra acting on itself via automorphisms.  As isomorphisms of lattices preserve all meets and joints, these are even automorphisms of complete boolean algebras, in particular locales. Locales in $\text{Set}^G$ are simply locales upon which $G$ acts by locale-automorphisms by C2.5.8(d) \cite{elephant} so the universal model is a  $[\text{Aut}(I,\lambda),\text{Set}]$-indexed locale. B2.3.10 \cite{elephant} states that, for any closure operator $j$ on a topos $\mathcal{S}$ an $\mathcal{S}$-indexed cocomplete category is a  $sh_j\mathcal{S}$-indexed cocomplete category iff it is a $j$-sheaf. Applying this result to the closure operator associated to the inclusion
...
\end{lemma}
\begin{proof} To see that this is a pullback, note that $\Gamma^{(G,\tau)}$ is a localic geometric morphism and pullbacks of such morphisms (along any geometric morphism) are again localic by B3.3.6 \cite{elephant}.  Denote by $(\mathcal{E},e,\Gamma)$ some choice of pullback of the square commuting up to natural iso $\eta$. Then it is determined by $\Gamma \Omega_\mathcal{E}$ which is a locale [find proper ref]. We know $f$ is open geometric as points of atomic toposes are always open surjective by C3.5.6 \cite{elephant}. By C3.1.28 \cite{elephant} $f$ is a left Beck-Chevalley morphism, hence we compute, denoting the subobject classifier of $\text{Sh}^{(G,\tau)}(\mathcal{B})$ by $\Omega$:
...
\indent Pullbacks of open surjections (along bounded geometric morphisms, so in particular localic geometric morphisms) are again open surjections by C3.1.23 \cite{elephant}, thus $\bar{f}$ has this property. 
\end{proof}
...
\end{corollary}
\begin{proof} Open surjections are descent morphisms by C5.1.6 \cite{elephant}.
\end{proof}
As before, to make this theory workable, we need a lot of different, insightful site presentations. There are two given in \cite{elephant}, first continuous equivariant sheaves A2.1.11(c)%is this a thing? Also C5.4.6
...
\end{lemma}
\begin{proof}
One just computes the Grothendieck construction "$\rtimes$" for the indexed category $U$ over $FinProb$ as explained in C2.5.4 \cite{elephant}. We use that $U(X,\mu)=\mathcal{P}(X)$ and $U(f):U(Y,\nu)\rightarrow U(X,\mu)$ is given by $f^{-1}:\mathcal{P}(Y)\rightarrow \mathcal{P}(X)$.\footnote{This had been remarked earlier and is fairly obvious. We might actually want to prove it explicitly anyway though.} Thus $FinProb\rtimes U$ has as objects as described in the lemma and as morphisms pairs $(f,\leq)$ where $f:(X,\mu, x)\rightarrow (Y,\nu, y)$ and $\leq: x\rightarrow f^{-1}(y)$ in $\mathcal{P}(X)$ this is equivalent to $f(x)\subseteq y$ in $\mathcal{P}(Y)$. Now we use the same line of argument as in C2.5.8(e) \cite{elephant}. Evidently, any morphism in $FinProb\rtimes U$ can be factored as follows:
\begin{center}
\begin{tikzcd}
...
\end{enumerate}
\end{lemma}
\begin{proof} Any sheaf has this properties. (i) is just how the canonical topology is defined and (ii) follows from III4 Lemma 2 of \cite{sheaves_geometry_logic}. This is not that hard to show.
\end{proof}
...
In this section we aim to ilustrate how we can use $\mathfrak{Prob}$ in order to look at problems in probability theory that have a more combinatorial and less analytic nature. 
...
% We need better notation for those coproducts

% TODO: This should be moved to the appropriate section:
\begin{definition}
    \begin{enumerate}
        \item $P^1_n := \{ (r_i) \in \mathbb{R}^n | \sum_{i=1}^n, r_i \geq 0 \}$
        \item $P^2_{r_i,s_i} := \{ (t_{ij}) \in \mathbb{R}^{n \times m} | \sum_{i=1}^n t_{ij} = s_j, \sum_{j=1}^m t_{ij} = r_i, t_{ij} \geq 0 \}$
        \item $P^2_{n,m} := \{ (t_{ij}) \in \mathbb{R}^{n \times m} | \sum_{i,j = 1}^{n,m} t_{ij} = 1 , t_ij \geq 0 \}$
    \end{enumerate}
\end{definition}
% TODO: Write out multiproduct using this, maybe write a more general definition, though not sure whether we need it.

\begin{definition}
    Let the sheaf of simple functions $\mathcal{S} \in Sh(FinProb, J_{at})$ be defined by
    \[
        \mathcal{S}(X,\mu) := \mathbb{R}^X,
    \]
    where we send a map $f: (X,\mu) \to (Y,\nu)$ to precomposition with $f$.
\end{definition}

We check the sheaf condition:

\begin{proposition}
    $S$ is a sheaf.
\end{proposition}
\begin{proof}
    We check the sheaf condition in \ref{sheaf_condition_weak} by checking that the fork
    \[
        \begin{tikzcd} \mathbb{R}^Y \arrow[r, "f^*"] & \mathbb{R}^X \arrow[r, "\pi_1", shift left] \arrow[r, "\pi_2"', shift right] &  \mathbb{R}^{X \times_f X}
        \end{tikzcd}
    \]
    is actually an equalizer. Since $f$ is surjective, $f^*$ is injective and moreover we know that its image consists of the $s \in \mathbb{R}^X$ that are constant on the fibers of $f$ (by fibers we mean $f^{-1}(y)$ for $y \in Y$).

    Thus it only remains to show that if for an $s \in \mathbb{R}^X$ we have that $\pi_1(s) = \pi_2(s)$, then $s$ is constant on the fibers of $f$.

    So assume that $\pi_1(s) = \pi_2(s)$. Then $s \circ \pi_1 = s \circ \pi_2$, which elementwise sais that for all $x,y \in X$ with $f(x) = f(y)$, we have that $s(x) = s(y)$. This implies that $s$ is constant on the fibers of $f$, concluding the proof.
\end{proof}

Using this, we can define simple functions over any interval.

\begin{definition}
    \begin{enumerate}
        \item Equip $S$ with $\mathbb{R}$-algebra structure, with poset structure and with unary relations indicating the expected value. Call such an object an expectation algebra. % TODO: Impose some axioms for the sake of an expectation algebra being a sensical concept.
...
In order to understand what the simple functions mean on the models, we need to decompose it into atoms, since then we can exploit that the inverse image preserves coproducts.

% TODO: We really need better notation for those coproducts......
\begin{proposition}
    $\mathcal{S} \cong \coprod_{n, r_i \in P^1_n, y_1 \le \cdots y_n} yU_{r_i}$
    % also describe this on the other symbols of an integration algebra.
\end{proposition}
\begin{proof}
    Check this objectwise and on morphisms.
\end{proof}

We check that this concept of a simple function corresponds to the classical concept that is given in \cite{fremlin}. % TODO: Elaborate on this and explain how fremlin shows that this abstract def of simple functions corresponds to the usual one on actual sigma-algebras.

\begin{proposition}
    Let $(A,\mu)$ be an interval in $Set$. Then the simple functions over $(A,\mu)$ can be given by:
\end{proposition}

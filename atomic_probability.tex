...
\[
A = \{(x_i) \in \prod_{i=1}^{\infty} I | \text{ finitely many } x_i \text{ nonzero or finitely many } x_i \text{ zero.}\}
\]
of the product of boolean algebras $\prod_{i \in \mathbb{N}} I$, where $(I, \lambda)$ is the Lebesgue interval. For some remarks on product boolean algebras, see \ref{partitions}. $A$ is boolean algebra(that is not complete) and we have a measure on this boolean algebra defined by
\[
...
\begin{enumerate}
\item Let $A$ be a complete boolean algebra. We say that a subset $S \subset A$ generates $A$ if $A$ is the only every complete boolean subalgebra of $A$ containing $S$.

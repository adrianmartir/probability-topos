...
% Some shortcuts
\newcommand{\ob}[1]{\textsf{ob}(#1)}
\newcommand{\id}[1]{id_{#1}}

\newcommand{\Hom}{\textsf{Hom}}
\newcommand{\Sh}{\textsf{Sh}}
\newcommand{\limit}{\textsf{lim}}
\newcommand{\eq}{\textsf{eq}}
\newcommand{\s}{\textsf{ }}

\newcommand{\Set}{\textsf{Set}}
\newcommand{\FinProb}{\textsf{FinProb}}
...
The representable functors are atoms in $\Prob$, i.e. objects whose only subobjects are the initial object and the object itself. A crucial property of an atomic topos (a topos of sheaves over a category equipped with the atomic topology) is that every object is a coproduct of atoms. So the natural question arises: Are the representables all atoms already? In \ref{atoms} we will give a complete list of the atoms and see that the representables are not all of them.


\subsection{Finite Limits}

Our goal in the next section will be to understand the logic of $\Prob$. Successfully doing so requires a very good understanding of what the flat functors on $\FinProb$ are and thus also requires a good understanding of finite limits in $\Prob$. This will be our goal in this subsection.
...
The most surprising part about multi-limits will be their nontrivial combinatorial content and their connection to polytopes. We will see that an overwhelming amount of statements in this paper (beyond statements about flat functors) are proved by using computations with multi-limits and polytopes.
...
\begin{definition}
    \begin{enumerate}
        ...
        \[
            \langle T(x) \mid x \in S \rangle := \coprod_{\substack{x \in S \\ y \in T(x)}} U_x .
        \]
        We will call an a object written in this notation a \emph{set of atoms}.
        \item We define an \emph{element} of a set of atoms $\langle T(x) \mid x \in S \rangle$ to be a tuple $(x,t) \in \coprod_{x \in S} T(x)$. If $T(x)$ is always the one-element set, we will abbreviate and say that $x$ is an \emph{element} of the set of atoms $\langle T(x) \mid x \in S \rangle$.
    \end{enumerate}
\end{definition}
...
\begin{example} \label{notation_example}
    \begin{itemize}
        ...
        \[ y U_{r_i} \times y U_{s_j} \cong \langle 1 \mid t_{ij} \in P^2_{r_i,s_j} \rangle, \]
        ...
        \[
            \prod_{j=1}^n y U_{r_i^j} \cong \langle 1 \mid t_{i^1 \cdots i^n} \in P^n_{r_i^1 \cdots r_i^n} \rangle
        \]
        ...
        \begin{equation*}
            ...
            &\langle 1 \mid t_{ij} \in P^2_{r_i, r_i'}, t_{ij} = 0 \text{ for all } i,j \text{ with } f(i) \neq g(j) \rangle
            \end{split}
        \end{equation*}
        \item Define the presheaf $U$ in the following way:
        \[ U \cong \langle 1 \mid (r_1,r_2) \in P^1_2 \rangle. \]
...
One should note that when specifying a sheaf $\mathcal{F}$ by using this set of atoms notation, the elements of $\mathcal{F}$ (as defined above) are precisely the monomorphisms $y U_{r_i} \hookrightarrow \mathcal{F}$. So one can think of this notation as providing us with a way of directly specifying sheaves through their generalized elements, without needing to worry about functoriality and sheaf conditions. Unfortunately one can't specify all the sheaves in this way. Nevertheless this notation can be easily adapted to incorporate the non-representable atoms we discuss in \ref{atoms}, but we won't need this in this article.
...
\begin{proposition}\label{atom_coprod_maps}
    Let $\mathcal{F} = \langle T(x) \mid x \in S \rangle$ and $\mathcal{G} = \langle T'(x) \mid x \in S' \rangle $. Then the maps $\mathcal{F} \to \mathcal{G}$ are precisely pairs $(g, (f_x))$, where $g$ is a map between the sets of elements
    \[
...
\end{tikzcd}
\]
is an equalizer diagram, where the $t_{ij}$ range over the elements of the multi-pullback $U_{r_i} \times_{U_{s_i}} U_{r_i}$ (the morphisms of the diagram are both $f$) and the projection $\pi_1$ and $\pi_2$ are induced by the universal 'projection maps' of multi-pullbacks.
\end{proposition}
\begin{proof}
...
\section{$\Prob$ as a classifying topos}
...
\begin{proof}
    ...
    \begin{enumerate}
        ...
        Intriguingly, we have shown that this map is simply a substitution. But we can give another substitution that can be checked to be an inverse of this map by using the boolean probability algebra axioms:
        \[
            [a_{ij} = b_i \wedge c_j]
        \]
...
\begin{proposition}
    We have that
    \[
        \mathcal{S} \cong \langle \{y_1 < \cdots < y_n \mid y_i \in \mathbb{R} \} \mid (r_1, \cdots, r_n) \in \mathcal{A}^{re} \rangle
    \]
...
\begin{align*}
    S \times S &\cong \langle \{y_1 < \cdots < y_n \mid y_i \in \mathbb{R} \} \mid (r_1, \cdots, r_n) \in \mathcal{A}^{re} \rangle^2 \\
    &\cong \langle \{y_1 < \cdots < y_n, z_1 < \cdots < z_m \mid y_i, z_j \in \mathbb{R} \} \mid (t_{ij})_{1 \leq i \leq n, 0 \leq j \leq m} \in \mathcal{A}^{re} \rangle
\end{align*}
...
\begin{proposition}
    \begin{enumerate}
        ...
        \[
            \langle \{y_1 < \cdots < y_n \mid y_i \in \mathbb{R}, \sum r_i y_i = r \} \mid (r_1, \cdots, r_n) \in \mathcal{A}^{re} \rangle
        \]
        of $\mathcal{S}$.
    \end{enumerate}
\end{proposition}
...
\end{proposition}
\begin{proof}
    The atomless measure algebra of $(X, \Sigma, \mu)$ is an inteval and thus can be viewed as a geometric morphism $F: \Set \to \Prob$. $F^*$ sends an atom $U_{r_i}$ to the set of partitions of the interval with measures $r_1,\cdots,r_n$. Thus, since $F^*$ is colimit preserving, $\mathcal{S}$ gets sent to
    \[
...
\[\int_X f(f_1,\cdots,f_n)d\mu=\\lim_{\substack{g_1\rightarrow f_1,\cdots, g_n\rightarrow f_n \text{ in }L^\infty\\ g_1\cdots g_n \text{ simple}}}  \int_Xf(g_1,\cdots,g_n)d\mu \]%formulate this properly

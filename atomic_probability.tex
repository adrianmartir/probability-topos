...
\[\forall f_1\cdots \forall f_n.( \Phi(f_1,\cdots, f_n)\implies \bigvee_{k\in \mathbb{N}}\forall g_1,\cdots, \forall g_n.(\bigwedge_{i=1}^n\bigvee_{\epsilon\leq 1/k}||f_i-g_i||_{\infty}=1/k\implies \Phi(g_1,\cdots g_n))) \]
holds in $\mathcal{S}$.\newline
...
\end{theorem}
\begin{proof}
$(L^\infty(X,\mu)\vDash \Phi)\iff (\mathcal{S}\vDash \Phi)$ as $\Phi$ is spreading and $S(X,\mu)$ is dense. Then apply the transfer lemma.
\end{proof}
